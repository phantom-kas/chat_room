...
$num_aff = $Db->query("UPDATE rooms set isclosed = 1 where id = ?",[$_POST['id']])->numAffectedRows();
if($num_aff ){
  echo json_encode(['message' => 'Room closed successfully', 'status' => 'success'], true);
  die();
}
echo json_encode(['message' => 'close Error', 'status' => 'error'], true);
die()
;
?>